...
| Choose applications | Generated profiles own readable `apps/homebrew-casks.nix`, `apps/nix-packages.nix`, and `apps/mac-app-store.nix`; the round-trip test proves declarations survive backup and restore. | `setup.sh --new` does not yet offer a beginner app-selection step, and the existing `dot apps` command writes the older `.local` layer rather than these profile-owned modules. | **Gap — not accepted** |
