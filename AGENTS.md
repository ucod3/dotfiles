...
3. **`.local/`** — current compatibility bridge for machine-local settings read
   by `lib/local.nix`; the migration direction is documented in `docs/PRODUCT.md`.
...
- **R4 — Do not casually alter the current `.local/` loader.** It protects live
  installations. Replace it only through an approved, compatibility-tested
  migration aligned with `docs/PRODUCT.md`.
...
2. **Blueprint** — R6. Include the clean-machine and existing-user paths.
3. **Implement** — stay in scope; preserve existing installations during migration.
